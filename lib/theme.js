...
/** Catch a photo that fails BEFORE React hydrates.
 *
 *  React's onError only works once the handler is attached, and on a slow
 *  connection an image has usually already failed by then — leaving the
 *  browser's broken-image glyph, which is exactly what the fallback exists to
 *  prevent. The old no-build site used an inline onerror="" attribute, which
 *  React will not render.
 *
 *  So: one listener, in the CAPTURE phase, installed before first paint.
 *  Error events do not bubble, but they do capture, which is why this works
 *  at the document level for every image on the page including ones drawn
 *  later.
 */
export const IMG_FALLBACK = `(function(){try{
  var BLANK='data:image/gif;base64,R0lGODlhAQABAIAAAAAAAP///yH5BAEAAAAALAAAAAABAAEAAAIBRAA7';
  document.addEventListener('error', function(e){
    var t=e.target;
    if(!t || t.tagName!=='IMG' || t.dataset.fellBack) return;
